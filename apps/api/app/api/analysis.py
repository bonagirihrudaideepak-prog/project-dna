...
from ..llm.prompts import PROMPT_VERSION, build_messages, validate_output
from ..llm.router import router_chat
from ..models import (
    Decision,
    DNAScore,
    Experiment,
...
from ..services.exports import to_json, to_print_html
from ..services.similarity import model_compatible, weighted_distance
from ..deps import optional_user, parse_id

router = APIRouter(tags=["analysis"])


def _snapshot(db: Session, snapshot_id: str) -> RepositorySnapshot:
...
def compare(body: CompareIn, user_id: str | None = Depends(optional_user), db: Session = Depends(get_db)):
    a = _snapshot(db, body.snapshot_a)
    b = _snapshot(db, body.snapshot_b)
...
def get_graph(snapshot_id: str, focus: str | None = None, depth: int = 1, user_id: str | None = Depends(optional_user), db: Session = Depends(get_db)):
    ...
    if focus:
        from ..analysis.graph.builder import query_bounded
...
def create_summary(snapshot_id: str, body: SummaryIn, user_id: str | None = Depends(optional_user), db: Session = Depends(get_db)):
    ...
    if settings.llm_provider_order.lower() not in ("none",):
        ...
        try:
            result = router_chat(system, user, timeout=settings.llm_timeout_seconds)
            parsed, status, reason = validate_output(result.text)
            if status == "ok":
                summary = {
                    "summary": parsed["summary"],
                    "claims": [
                        {"text": h, "evidence_ids": []} for h in parsed["highlights"][: body.max_claims]
                    ],
                ...
                _persist_llm_run(db, s.id, "summary", result.provider, result.model, system, user, parsed, "ok")
            else:
                summary = fallback
                summary["llm_note"] = f"{result.provider}: {reason} (used deterministic fallback)"
                provider_used = result.provider
                model_used = result.model
                _persist_llm_run(db, s.id, "summary", result.provider, result.model, system, user, {"error": reason}, "unvalidated")
        except Exception as exc:  # noqa: BLE001 - fail over to deterministic
            summary = fallback
            summary["llm_note"] = f"LLM unavailable ({type(exc).__name__}); used deterministic fallback"
