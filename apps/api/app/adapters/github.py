...
from .errors import DNAError, RateLimitedError
...
class GitHubAdapter:
    ...
    async def _get(self, url: str, params: dict | None = None) -> Any:
        ...
        if resp.status_code == 403:
            raise RateLimitedError(retry_after=str(resp.headers.get("x-ratelimit-reset")))
        if resp.status_code == 404:
            raise DNAError(message="Repository not found or not accessible.", code="REPO_NOT_FOUND")
        if resp.status_code == 401:
            raise DNAError(message="GitHub authentication failed.", code="AUTH_FAILED")
        if resp.status_code >= 400:
            raise DNAError(message=f"GitHub request failed: {resp.status_code}", code="GITHUB_REQUEST_FAILED", retryable=True)
        return resp.json()
    ...
    async def list_repositories(self, q: str = "", page: int = 1, per_page: int = 30) -> dict[str, Any]:
        ...
        data = await self._get("/user/repos", {"page": page, "per_page": per_page, "visibility": "public"})
    ...
    async def archive_bytes(self, full_name: str, sha: str) -> bytes:
        ...
        if resp.status_code != 200:
            raise DNAError(message=f"Archive download failed: {resp.status_code}", code="ARCHIVE_DOWNLOAD_FAILED", retryable=True)
        return resp.content
